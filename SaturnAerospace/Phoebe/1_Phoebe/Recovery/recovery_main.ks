...
GLOBAL FUNCTION _REENTRYBURN {
    IF _RECOVERYMETHOD = "ASDS" {
        ...
        UNTIL ship:verticalSpeed >= _ASDS_REENTRYSTOP {wait 0.}
...
GLOBAL FUNCTION _LANDINGBURNRTLS {
    lock _TRUERADAR to alt:radar - _BOOSTER_ALTOFFSET.
    ...
    // Landing Burn Start
        UNTIL _TRUERADAR <= (_STOPDISTANCE - _BOOSTER_ALTOFFSET + 250) {print _TRUERADAR at (10, 13). wait 0.} // Wait until suicide burn altitude#
    ...
    // Landing Gear
        IF addons:tr:hasimpact {
            WHEN alt:radar < 200 then {_DEPLOYLANDINGLEGS(). set _MAXAOA to -1.5.}
        } 
    ...
    // Final Landing Sector
        IF _TRUERADAR < 80 {lock steering to up.}
